...
import info.vircletrx.agents.service.ChallengeReport;
...
/**
 * Base class to monitor the Tronx blockchain for superblock-related events
...
public abstract class SuperblockBaseClient {
    ...
    protected final TrxWrapper trxWrapper;
        ...
        public void run() {
            try {
                if (!trxWrapper.isTrxNodeSyncing()) {
                    ...
                    long toBlock = trxWrapper.getTrxBlockCount() - agentConstants.getConfirmations(agentRole) + 1;
                    long fromBlock;
                    if(latestEthBlockProcessed == 0)
                        fromBlock = 0; // toBlock - 5000;    // john
                    else
    ...
    protected final boolean arePendingTransactions() throws InterruptedException, IOException {
        return agentRole == AgentRole.SUBMITTER
                ? trxWrapper.arePendingTransactionsForSendSuperblocksAddress()
                : trxWrapper.arePendingTransactionsForChallengerAddress();
    }
