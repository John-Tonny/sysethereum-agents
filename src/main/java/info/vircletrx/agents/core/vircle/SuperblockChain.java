...
public class SuperblockChain {
    ...
    private final VircleWrapper vircleWrapper; // Interface with the Vircle blockchain
     ...
     * @param allVircleHashesToHash All the Vircle blocks that come after the last block of the last stored superblock.
     ...
     * @throws Exception
     */
    public void storeSuperblocks(Stack<Sha256Hash> allVircleHashesToHash, Keccak256Hash initialPreviousSuperblockHash)
            throws Exception {
        Context.propagate(vircleContext);
        if (allVircleHashesToHash.empty())
            return;

        List<Sha256Hash> nextSuperblockVircleHashes;
        ...
        while (!allVircleHashesToHash.empty()) {
            // Modify allVircleHashesToHash and get hashes for next superblock.
            nextSuperblockVircleHashes = popBlocksBeforeTime(allVircleHashesToHash, getStoringStopTime());
            // if we don't have a collection of 60 blocks that are at least 1 hour old we exit
            if(nextSuperblockVircleHashes.isEmpty()){
                break;
            }
            StoredBlock nextSuperblockLastBlock = vircleWrapper.getBlock(
                    nextSuperblockVircleHashes.get(nextSuperblockVircleHashes.size() - 1));

            Superblock newSuperblock = superblockFactory.make(
                    merkleRootComputer.computeMerkleRoot(nextSuperblockVircleHashes),
                    nextSuperblockVircleHashes,
                    nextSuperblockLastBlock.getHeader().getTimeSeconds(),
                    vircleWrapper.getMedianTimestamp(nextSuperblockLastBlock),
            ...
            nextSuperblockVircleHashes.clear();
        }
    }
     ...
     * @param hashStack All the Vircle blocks that come after the last block of the last stored superblock.
     *                  Must not be empty.
     *                  Modified by function: all the blocks up to and not including the first ('highest') one
    ...
    private List<Sha256Hash> popBlocksBeforeTime(Stack<Sha256Hash> hashStack, Date endTime) throws Exception {
        ...
        while (!hashStack.empty() && new Date(vircleWrapper.getMedianTimestamp(vircleWrapper.getBlock(hashStack.peek()))*1000L).before(endTime)) {
    ...
    public Superblock findBySysBlockHash(Sha256Hash blockHash) {
        ...
        while (sb != null) {
            if (sb.hasVircleBlock(blockHash))
                return sb;
    ...
    public boolean sendingTimePassed(Superblock superblock) {
        return new Date(superblock.getLastVircleBlockMedianTime()*1000L).before(getSendingStopTime());
    }
}