...
public class SuperblockSerializationHelper {
    ...
    private static final int VIRCLE_BLOCK_HASHES_PAYLOAD_OFFSET = NUMBER_OF_HASHES_PAYLOAD_OFFSET + UINT32_LENGTH;
    ...
    public SuperblockData fromBytes(byte[] payload) {
        ...
        long lastVircleBlockTime = new BigInteger(Utils.reverseBytes(SuperblockUtils.readBytes(
                payload, LAST_BLOCK_TIME_PAYLOAD_OFFSET, BIG_INTEGER_LENGTH))).longValue();
        long lastVircleBlockTimeMTP = new BigInteger(Utils.reverseBytes(SuperblockUtils.readBytes(
                payload, LAST_BLOCK_TIME_MTP_PAYLOAD_OFFSET, BIG_INTEGER_LENGTH))).longValue();
        Sha256Hash lastVircleBlockHash = Sha256Hash.wrapReversed(SuperblockUtils.readBytes(
                payload, LAST_BLOCK_HASH_PAYLOAD_OFFSET, HASH_BYTES_LENGTH));
        long lastVircleBlockBits = Utils.readUint32(payload, LAST_BLOCK_BITS_PAYLOAD_OFFSET);
        ...
        long numberOfVircleBlockHashes = Utils.readUint32(payload, NUMBER_OF_HASHES_PAYLOAD_OFFSET);
        List<Sha256Hash> vircleBlockHashes = deserializeHashesLE(payload, VIRCLE_BLOCK_HASHES_PAYLOAD_OFFSET, numberOfVircleBlockHashes);

        return new SuperblockData(merkleRoot, vircleBlockHashes, lastVircleBlockTime, lastVircleBlockTimeMTP, lastVircleBlockBits, lastVircleBlockHash, parentId, height);
    }
    ...
    public void serializeLE(SuperblockData sbd, OutputStream stream) throws IOException {
        ...
        stream.write(Utils.reverseBytes(SuperblockUtils.toBytes32(sbd.lastVircleBlockTime))); // 32
        stream.write(Utils.reverseBytes(SuperblockUtils.toBytes32(sbd.lastVircleBlockTimeMTP))); // 32
        stream.write(sbd.lastVircleBlockHash.getReversedBytes()); // 32
        stream.write(Utils.reverseBytes(SuperblockUtils.toUint32(sbd.lastVircleBlockBits))); // 4
    ...
    public void serializeBE(SuperblockData sb, OutputStream stream) throws IOException {
        ...
        stream.write(SuperblockUtils.toBytes32(sb.lastVircleBlockTime));
        stream.write(SuperblockUtils.toBytes32(sb.lastVircleBlockTimeMTP));
        stream.write(sb.lastVircleBlockHash.getBytes());
        stream.write(SuperblockUtils.toUint32(sb.lastVircleBlockBits));
    ...
    public void serializeForStorage(SuperblockData sbd, OutputStream stream) {
        try {
            ...
            Utils.uint32ToByteStreamLE(sbd.vircleBlockHashes.size(), stream);
            serializeHashesLE(sbd.vircleBlockHashes, stream);
        } catch (IOException e) {
     ...
     * @param hashes List of Vircle block hashes.
     * @param stream Output stream where the information will be written. Modified by the function.
     * @throws IOException
     */
     ...
     * deserializes its Vircle block hashes and returns them in a list.
